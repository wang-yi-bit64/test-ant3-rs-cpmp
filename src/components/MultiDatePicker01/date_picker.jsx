...
function DatePicker(
  ...
  const [date, setDate] = useState(),
    ...
    closeCalendar = useCallback(() => {
      ...
      const inputs = getInputs(inputRef);

      if (inputs) inputs.forEach((input) => input.blur());

      if (ref.current.mobile) {
        const popper = calendarRef.current.parentNode.parentNode;
...
function getStringDate(date, separator) {
  const dates = [].concat(date).map(toString);
